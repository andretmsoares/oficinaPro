...
class OrdemDeServicoControllerTest {
  ...
  void deveBuscarOsPorId() throws Exception {
    ...
    mockMvc
        .perform(get("/api/ordens-servico/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(1))
  ...
  void deveAtualizarStatusOs() throws Exception {
    ...
    AtualizarStatusOSRequestDTO statusRequest =
        new AtualizarStatusOSRequestDTO(StatusOrdemDeServico.EM_EXECUCAO);

    when(service.atualizarStatus(eq(1L), any(AtualizarStatusOSRequestDTO.class)))
        .thenReturn(osEmExecucao);
  ...
  void deveAtribuirMecanicoNaOs() throws Exception {
    ...
    when(service.atribuirMecanico(eq(1L), any(AtribuirMecanicoRequestDTO.class)))
        .thenReturn(responseDTO);

    mockMvc
        .perform(
            patch("/api/ordens-servico/1/mecanico")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
