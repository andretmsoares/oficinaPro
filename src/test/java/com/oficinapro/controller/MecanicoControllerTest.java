...
class MecanicoControllerTest {
  ...
  void deveNegarAcessoParaMecanico() throws Exception {
    mockMvc.perform(get("/api/mecanicos")).andExpect(status().isForbidden());
  }
  ...
  void deveRetornar404AoBuscarMecanicoInexistente() throws Exception {
    ...
    mockMvc.perform(get("/api/mecanicos/99")).andExpect(status().isNotFound());
  }
  ...
  void deveDeletarMecanico() throws Exception {
    ...
    mockMvc.perform(delete("/api/mecanicos/1").with(csrf())).andExpect(status().isNoContent());
  }
}
