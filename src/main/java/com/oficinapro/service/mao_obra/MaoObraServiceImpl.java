...
public class MaoObraServiceImpl implements MaoObraService {
  ...
  public List<MaoObraResponseDTO> listarPorOrdemServico(Long osId) {
    ...
    return maoObraRepository.findByOrdemDeServicoId(osId).stream().map(this::toResponse).toList();
  }
  ...
  private MaoObra buscarPorEntidadeId(Long id) {
    MaoObra maoObra =
        maoObraRepository.findById(id).orElseThrow(() -> new MaoObraNotFoundException(id));
